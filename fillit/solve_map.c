...
/*   Updated: 2019/03/27 22:48:46 by maparmar         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "fillit.h"

void    put_token(t_tetris *tetri, t_map *map, point *p, char c) // put piece/token on the map to the character specified.
{
    int i;
    int j;
...
int     poistion_check(t_tetris *tetri, t_map *map, int x, int y) // boundry condition check
{
    int i;
    int j;
    ...
    while(i < tetri->breadth)
    {
        ...
        while(j < tetri->length)
        {
            if(tetri->piece_pos[j][i] == '#' && (map->arr[y + j][x + i] != '.'))
            {
                return (0);
            }
            j++;
